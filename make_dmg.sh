...
DMG_NAME="${APP_NAME}_v3.0.0.dmg"
DMG_DIST="dist/${DMG_NAME}"

# 1. 检查必备工具
if ! command -v create-dmg &> /dev/null; then
    echo "未检测到 [create-dmg] 工具。"
    echo "正在为您尝试通过 Homebrew 安装 (可能需要您的账户密码)..."
...
if [ $RESULT -eq 0 ] || [ -f "$DMG_DIST" ]; then
    ...
    if [ $RESULT -ne 0 ]; then
        echo "提示：检测到 Finder 布局微调被系统拦截 (Error -10006)。"
